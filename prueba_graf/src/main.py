...
if __name__ == "__main__":
    for symbol in parameters.symbols:
        for timeframe in parameters.timeframes:
            print(f"Procesando {symbol} para timeframe {timeframe}")
            df = utility.csv_to_pd(f"{symbol}_{timeframe}")
            if df is None:
                print(f"⚠️ No se pudo cargar el CSV para {symbol} {timeframe}")
                continue
            ...
            tendencia = utility.determinar_tendencia_5(df, 3)
            ...
            utility.save_bars_csv(symbol, timeframe, df)

            #utility.graficar_pivots_soportes_resistencias(df, df_pivots, soportes, resistencias, "6A", tendencia, rsi, name="6A", timeframe="15m")
            #utility.graficar_pivots_soportes_resistencias_2(df, soportes, resistencias, symbol, tendencia, rsi, name=symbol, timeframe=timeframe)

            utility.write_db(symbol, timeframe, tendencia, float(rsi), soportes, resistencias)